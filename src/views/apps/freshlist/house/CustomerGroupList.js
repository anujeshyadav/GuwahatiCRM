...
import { Eye, ChevronDown, Edit, CornerDownLeft, Trash } from "react-feather";
                    ...
                    <Edit
                      ...
                      onClick={() =>
                        this.props.history.push(
                          `/app/Ajgroup/account/CreateCustomerGroup/${params?.data?._id}`
                        )
                      }
                    />
                  )}
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="d-flex align-items-center cursor-pointer">
                <div>
                  <span>{params.data?.groupName}</span>
                </div>
              </div>
            );
          },
    ...
    await _Get(View_CustomerGroup, db)
      .then((res) => {
        ...
        if (res?.CustomerGroup) {
          ...
          if (myActive?.length) {
            this.setState({ rowData: myActive });
          }
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("customerGroup"));
